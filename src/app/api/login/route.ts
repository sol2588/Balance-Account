...
import { generateAccess, generateRefresh, accessVerify, refreshVerify } from "@/utils/jwtToken";
...
export async function POST(req: NextRequest) {
  if (req.method == "POST") {
    ...
    const checkUser = await getDoc(userDocRef);
    if (!checkUser.exists()) {
      return NextResponse.json({ message: "존재하지 않는 계정 입니다." }, { status: 400 });
    }

    // 3) 입력한 비밀번호가 올바르지 않는 경우 - checkUser.data()는 값을 가져오기 때문에 boolean 처리 방식으로 exists()
    ...
    if (checkUser.exists() && checkHash) {
      ...
      const pinExist = checkUser.data().pinNum ? true : false;
      const name = checkUser.data().name;
